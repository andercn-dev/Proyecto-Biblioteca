...
{
    private $usuario, $nombre, $clave, $id, $estado, $dni, $apellido, $telefono, $fecha_nacimiento, $correo, $carrera;
    ...
    public function getEstudiante($correo, $clave) {
        $sql = "SELECT * FROM estudiante WHERE correo = '$correo' AND clave = '$clave' AND estado = 1";
    ...
    public function registrarUsuarioB($dni, $nombre, $apellido, $telefono, $fecha_nacimiento, $correo, $carrera, $clave)
    {
        $this->dni = $dni;
        $this->nombre = $nombre;
        ...
        $this->clave = $clave;
        $verificar = "SELECT * FROM estudiante WHERE dni = '$this->dni'";
        $existe = $this->select($verificar);
        if (empty($existe)) {
            $sql = "INSERT INTO estudiante(dni, nombre, apellido, telefono, fecha_nacimiento, correo, carrera, clave) VALUES (?,?,?,?,?,?,?,?)";
            $datos = array($this->dni, $this->nombre, $this->apellido, $this->telefono, $this->fecha_nacimiento, $this->correo, $this->carrera, $this->clave);
            $data = $this->save($sql, $datos);
            if ($data == 1) {
                $res = "ok";
            } else {
